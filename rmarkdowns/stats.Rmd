...
ggplot(sdr_data, aes(x = goal_9_score, y = goal_17_score, label = country)) +
  geom_point() +
  geom_smooth() +
  stat_cor() +
  theme_minimal()
...
# The countries above the line of best fit are are doing well in terms of high scores over 50 and 60. If countries are doing well in innovation, infrastructure, and technology, then there will be more countries who do well with SDG 9 and SDG 17. This going for a positive correlation. Countries who are outliers will do well in one SDG goal rather than 2 SDG goals. 
      
      - are there any points that don't follow the trend (outliers)
# Yes, there are points that don't follow the tred. Such as points that are doing extremely well in the scores of SDG 9 but are around the score of 20 for SDG 17. 
        - which country(s) are outliers?
# The country that are outliers are the countries at the bottom.