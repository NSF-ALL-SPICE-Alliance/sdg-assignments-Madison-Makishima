...
```{r}
country_clusters <- as.data.frame(k4$cluster)
```
...
```{r}
rf_matmort <- randomForest(normalized_score_sdg3_matmort ~ ., data = sdr_data_imputed, importance = TRUE)
```

```{r}
rf_matmort <- randomForest(normalized_score_sdg9_matmort ~ ., data = sdr_data_imputed, importance = TRUE)
```
```{r}
normalized_score_sdg9_matmort <- randomForest(normalized_score_sdg9_matmort ~ .,
                             data = sdr_data_imputed,
                             importance = TRUE)
```
