...
```
```{r}
table(sdr_data$goal_5_dash, sdr_data$regions_used_for_the_sdr)
```
...
```{r}
SDG_Score == sdr_data %>%
select(goal_14_score, goal_17_score)
```
...
```{r}
SDG_scores <- sdr_data %>% 
  select(goal_5_score, goal_3_score)
```
...
sdr_data %>% 
  group_by(regions_used_for_the_sdr) %>% 
  summarise(mean_goal_14_score_by_region = mean(population_using_at_least_basic_drinking_water_services_percent))
```
