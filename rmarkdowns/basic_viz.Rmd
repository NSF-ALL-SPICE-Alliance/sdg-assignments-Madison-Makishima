...
Welcome to Basic Visualizations! We're going to go over some code on how to create our first visualizations using our SDR 2023 Data.an
...
```{r}
ggplot(data = sdr_data, aes(x= goal_14_score, y = goal_17_score)) +
  geom_point()
```

```{r}
ggplot(data = sdr_data, aes(x= goal_14_score, y = goal_17_score, color = regions_used_for_the_sdr)) +
  geom_point()
```

```{r}
ggplot(data = sdr_data, aes(x= goal_14_score, y = goal_17_score, color = goal_17_score)) +
  geom_point()
```

##MADISON'S INTERPRETATION:
# I used SDG 14 as my x-axis and SDG 17 as my y-axis. In the last gradient scatterplot, the countries who are attempting to go over the score of 60 for SDG 17 are represented by the dark blue dots. This means that there still needs to be more partnership done. For SDG 14, there is a lot of dark blue dots representing the countries that need to do more work. As for the light blue dots, they represent the countries that are doing well for SDG 17 from the scores of 70 to over 90 on the y axis. The countries for the x-axis are fewer when it comes to achieving SDG 14. 
...
```{r}
ggplot(sdr_data, aes(x= goal_14_score, y = country)) +
  geom_bar(stat = "identity")
```
...
```{r}
ggplot(mena_sdr_data, aes(x= goal_14_score, y = country)) +
  geom_bar(stat = "identity") 
```
...
##MADISON'S INTERPRETATION:
# In both the gray and rainbow bar graph, around 17 countries fall in the scores of 65 to 70 range for SDG 14. The second highest number of countries that are doing well for SDG 14 is around 13 with the scores of 60 to 65. The lowest number of countries from the MENA region that's doing well over the score of 90 is around 2 countries.